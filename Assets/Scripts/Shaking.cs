...
public class Shaking : MonoBehaviour {
    ...
    void Update()
    {
        float passed = Passed(); // Наш прогресс

        // Позиция в Vector3, к которой мы стримимся
        Vector3 GoalPos = new Vector3(MyTransform.localPosition.x, passed * AmplitudeHeight + DefCamPos, MyTransform.localPosition.z);
        ...
        {
            passed = 0; // Только если мы не идем в бок
        }
        Vector3 GoalRot = new Vector3(MyTransform.localPosition.x, MyTransform.localPosition.y, passed * AmplitudeRot + DefCamRot);
